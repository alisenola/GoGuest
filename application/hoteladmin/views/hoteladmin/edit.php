...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            <?php echo $lang->language["hotel_user_edit"]; ?>
                ...
                <div class="box box-primary">
                    ...
                    <form role="form" action="<?php echo base_url() . "hoteladmin/update" ?>" method="post" enctype="multipart/form-data" onsubmit="return checkalldata();" >
                        <div class="box-body">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="type" id="type" onchange="showcat()">
                                    <option value="admin" <?php if ($hoteluser[0]['type'] == 'admin') { ?> selected="selected" <?php } ?>><?php echo $lang->language["hotel_user_type_admin"]; ?></option>
                                    <option value="manager" <?php if ($hoteluser[0]['type'] == 'manager') { ?> selected="selected" <?php } ?>><?php echo $lang->language["hotel_user_type_manager"]; ?></option>

                                </select>                                

                            </div>
                            <div class="form-group" id="categoryuniqiddiv" <?php if ($hoteluser[0]['type'] == 'admin') { ?> style="display: none;" <?php } ?>>
                                ...
                                <?php if (count($category) > 0) { ?>
                                    <select class="form-control" name="categoryuniqid" id="categoryuniqid">
                                        ...
                                        for ($i = 0; $i < count($category); $i++) {
                                            $sel = '';
                                            if ($category[$i]['categoryuniqid'] == $hoteluser[0]['categoryuniqid']) {
                                                $sel = 'selected = "selected"';
                                            } else {
                                                $sel = '';
                                            }
                                            ?>
                                            <option value="<?php echo $category[$i]['categoryuniqid']; ?>" <?php echo $sel ?>><?php echo $category[$i]['categoryname']; ?></option>
                                        <?php }
                                        ?>
                                    </select>
                                    <?php
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="username" name="username" value="<?php echo $hoteluser[0]['username']; ?>">
                            </div>
                            <div class="form-group">
                                <label for=""><?php echo $lang->language["hotel_email"]; ?></label>
                                <input type="text" class="form-control" id="hotelemail" name="hotelemail" value="<?php echo $hoteluser[0]['useremail']; ?>">
                            </div>
                        ...
                        <div class="box-footer">
                            <input type="hidden" name="huid" value="<?php echo $hoteluser[0]['huid']; ?>">
