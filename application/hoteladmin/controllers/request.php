...
class Request extends CI_Controller {
	...
	public function index()
	{
            ...
            $this->data['requests'] = $this->requests->get_all_request();
            //echo $this->db->last_query();
            //echo '<pre>'; print_r($this->data['requests']); die();
            $this->paging['base_url'] = site_url() . "request/index";
            $this->paging['uri_segment'] = 3;
            $this->paging['total_rows'] = count($this->requests->get_all_request());
       ...
       //$last_id = 1;
        $requestdata = $this->requests->get_all_new_request($last_id);
        $data['html'] = '';
        $data['id'] = $last_id ;
        if (count($requestdata) > 0) {
            $data['id'] =$requestdata[0]['requestid'];
            for ($i = 0; $i < count($requestdata); $i++) {
                $data['html'] .= "<tr>
            <td>" . $requestdata[$i]['userfirstname'] . " </td>
            <td>" . $requestdata[$i]['roomno'] . " </td>
            <td> " . $requestdata[$i]['categoryname'] . " </td>
            <td> " . $requestdata[$i]['requestdescription'] . "</td>
            <td> " . $requestdata[$i]['status'] . "  </td>
        </tr>";
            }
